...
from game_logic.aida_game import AidaGame
...
class AidaBot(commands.Cog):
    def __init__(self, bot):
        """
        Initializes the AidaBot cog.
        """
        self.bot = bot
        self.active_games = {}
    ...
    async def start_aida_game(self, interaction: discord.Interaction, player_name: str = None):
        """
        Starts a new game against Aida.
        """
        ...
        thread = await interaction.channel.create_thread(
            name=f"🧠 Aida-vs-{player_name}",
            type=discord.ChannelType.public_thread,
            reason="New Aida Game",
        )

        await interaction.followup.send(f"🧠 Aida game started in {thread.mention}", ephemeral=True)

        game = AidaGame(self.bot, interaction, thread, player_name)
        ...
        intro = [
            "🧠 Welcome, challenger to Ape In!",
            "⚙️ You'll compete against me — Aida, the Oracle of Insight.",
    ...
    async def aida_turn(self, user_id):
        """
        Handles Aida's turn.
        """
        thread = self.active_games[user_id]["thread"]
        ...
        await thread.send("🤖 Aida is drawing cards...")

        actions, aida_won = game.aida_play_turn()
        for msg in actions:
        ...
        await thread.send(
            ...
            f"🤖 Aida: `{game.aida_score} sats`"
        )

        if aida_won or game.is_game_over():
            await self.end_game(user_id)
        else:
            await self.begin_player_turn(user_id)
    ...
    async def end_game(self, user_id):
        ...
        aida_score = game.aida_score

        # Determine the winner
        if player_score >= self.WINNING_SCORE or player_score > aida_score:
            winner = player_name
        elif aida_score >= self.WINNING_SCORE or aida_score > player_score:
            winner = "Aida"
        else:
            winner = "Draw"
        ...
        try:
            await record_game_result(
                ...
                winner_name=winner,
                score=max(player_score, aida_score),
                mode="Aida",
                participant_ids=[user_id, "Aida"],
                participant_names=[player_name, "Aida"]
            )
        except Exception as e:
        ...
        try:
            await post_results_to_channel(
                self.bot,
                winner_name=winner,
                total_score=max(player_score, aida_score),
                game_mode="Aida",
                player_names=[player_name, "Aida"],
                player_name=player_name
            )
        except Exception as e:
    ...
    async def on_player_forfeit(self, user_id, interaction):
        ...
        await thread.send("🏳️ You forfeited the match. Aida wins by default.")

        try:
            await post_results_to_channel(
                self.bot,
                winner_name="Aida",
                total_score=game.aida_score,
                game_mode="Aida",
                player_names=[player_name, "Aida"],
                player_name=player_name
            )
    ...
    async def handle_idle_forfeit(self, user_id):
        """
        Handles idle timeout by ending the player's turn and moving to Aida's turn.
        """
        ...
        await thread.send(f"🕒 {player_name}, you were idle too long. Your turn has ended, and Aida will now play.")
...
async def start_aida_game(interaction: discord.Interaction, player_name: str = None):
    bot_cog = interaction.client.get_cog("AidaBot")
    if bot_cog:
        await bot_cog.start_aida_game(interaction, player_name)
    else:
        print("❌ AidaBot cog not found.")
        try:
            await interaction.followup.send("🚨 Could not find AidaBot cog.", ephemeral=True)
...
async def setup(bot):
    """
    Sets up the AidaBot cog.
    """
    await bot.add_cog(AidaBot(bot))