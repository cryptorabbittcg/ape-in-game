...
from game_logic.lana_game import LanaGame
...
class LanaBot(commands.Cog):
    def __init__(self, bot):
        """
        Initializes the LanaBot cog.
        """
        self.bot = bot
        self.active_games = {}
    ...
    async def start_lana_game(self, interaction: discord.Interaction, player_name: str = None):
        """
        Starts a new game against Lana.
        """
        ...
        thread = await interaction.channel.create_thread(
            name=f"🧠 Lana-vs-{player_name}",
            type=discord.ChannelType.public_thread,
            reason="New Lana Game",
        )

        await interaction.followup.send(f"🧠 Lana game started in {thread.mention}", ephemeral=True)

        game = LanaGame(self.bot, interaction, thread, player_name)
        ...
        intro = [
            "🤖 Initiating logic sequence for Ape In!",
            "🤖 I'm your opponent: Lana, the probability tactician!",
    ...
    async def lana_turn(self, user_id):
        """
        Handles Lana's turn.
        """
        thread = self.active_games[user_id]["thread"]
        ...
        await thread.send("🤖 Lana is drawing cards...")

        actions, lana_won = game.lana_play_turn()
        for msg in actions:
        ...
        await thread.send(
            ...
            f"🤖 Lana: `{game.lana_score} sats`"
        )

        if lana_won or game.is_game_over():
            await self.end_game(user_id)
        else:
            await self.begin_player_turn(user_id)
    ...
    async def end_game(self, user_id):
        ...
        lana_score = game.lana_score

        # Determine the winner
        if player_score >= self.WINNING_SCORE or player_score > lana_score:
            winner = player_name
        elif lana_score >= self.WINNING_SCORE or lana_score > player_score:
            winner = "Lana"
        else:
            winner = "Draw"
        ...
        try:
            await record_game_result(
                ...
                winner_name=winner,
                score=max(player_score, lana_score),
                mode="Lana",
                participant_ids=[user_id, "Lana"],
                participant_names=[player_name, "Lana"]
            )
        except Exception as e:
        ...
        try:
            await post_results_to_channel(
                self.bot,
                winner_name=winner,
                total_score=max(player_score, lana_score),
                game_mode="Lana",
                player_names=[player_name, "Lana"],
                player_name=player_name
            )
        except Exception as e:
    ...
    async def on_player_forfeit(self, user_id, interaction):
        ...
        await thread.send("🏳️ You forfeited the match. Lana wins by default.")

        try:
            await post_results_to_channel(
                self.bot,
                winner_name="Lana",
                total_score=game.lana_score,
                game_mode="Lana",
                player_names=[player_name, "Lana"],
                player_name=player_name
            )
    ...
    async def handle_idle_forfeit(self, user_id):
        """
        Handles idle timeout by ending the player's turn and moving to Lana's turn.
        """
        ...
        await thread.send(f"🕒 {player_name}, you were idle too long. Your turn has ended, and Lana will now play.")
...
async def start_lana_game(interaction: discord.Interaction, player_name: str = None):
    bot_cog = interaction.client.get_cog("LanaBot")
    if bot_cog:
        await bot_cog.start_lana_game(interaction, player_name)
    else:
        print("❌ LanaBot cog not found.")
        try:
            await interaction.followup.send("🚨 Could not find LanaBot cog.", ephemeral=True)
...
async def setup(bot):
    """
    Sets up the LanaBot cog.
    """
    await bot.add_cog(LanaBot(bot))