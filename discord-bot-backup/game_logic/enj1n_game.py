...
class Enj1nGame:
    def __init__(self, bot, interaction, thread, player_name, round_limit=15, winning_score=300):
        """
        Initializes the Enj1nGame instance.
        """
        self.bot = bot
    ...
    def enj1n_roll_die(self):
        """
        Rolls a weighted die for En-J1n.
        """
    ...
    def handle_bearish(self, card, is_player=True):
        ...
        """
        roll = self.roll_die() if is_player else self.enj1n_roll_die()
        dodged = roll % 2 == 0
        ...
        if dodged:
            ...
            if card.penalty == "Reset":
                ...
                if is_player:
                    ...
                    self.enj1n_score = max(0, self.enj1n_score - 10)
    ...
    def enj1n_play_turn(self):
        """
        Handles En-J1n's turn logic.
        """
        self.enj1n_turn_score = 0
        actions = []
        ...
        while True:
            ...
            if card.type == "Bearish":
                ...
                if not success:
                    actions.append(f"En-J1n drew a {card.name} (Bearish) and rolled a {roll} → {phrase}. Turn ends.")
                    self.enj1n_turn_score = 0
                    break
                else:
                    actions.append(f"En-J1n drew a {card.name} (Bearish) and rolled a {roll} → {phrase}.")
                    continue

            roll = self.enj1n_roll_die()

            if roll == 1:
                actions.append(f"En-J1n drew a {card.name} ({card.value} pts) and rolled a 1 → Busted! Turn ends.")
                self.enj1n_turn_score = 0
                break
            else:
                self.enj1n_turn_score += card.value
                actions.append(f"En-J1n drew a {card.name} ({card.value} pts) and rolled a {roll} → Turn score: {self.enj1n_turn_score}")

                behind_by = self.player_score - self.enj1n_score
                if behind_by > 20:
                    actions.append("En-J1n stacks aggressively to catch up.")
                    self.enj1n_score += self.enj1n_turn_score
                    break
                elif self.enj1n_turn_score >= 50:
                    actions.append("En-J1n stacks at 50 sats.")
                    self.enj1n_score += self.enj1n_turn_score
                    break
                else:
                    risk_roll = random.uniform(0, 100)
                    if risk_roll > 25:  # 75% chance to continue
                        continue
                    else:
                        actions.append("En-J1n decides to stack early.")
                        self.enj1n_score += self.enj1n_turn_score
                        break

        return actions, self.enj1n_score >= self.winning_score
    ...
    def is_game_over(self):
        ...
        return (
            ...
            self.enj1n_score >= self.winning_score or
    ...
    def get_winner(self):
        ...
        """
        if self.player_score > self.enj1n_score:
            return "player", "You win!"
        elif self.enj1n_score > self.player_score:
            return "enj1n", "En-J1n wins!"
        else:
    ...
    async def conclude_game(self):
        ...
        result_message = (
            ...
            f"🤖 En-J1n wins! {phrase}" if winner == "enj1n" else
            "🤝 It's a tie!"
        )
        ...
        try:
            await post_results_to_channel(
                bot=self.bot,
                winner_name=self.player_name if winner == "player" else "En-J1n",
                total_score=max(self.player_score, self.enj1n_score),
                game_mode="En-J1n",
                player_names=[self.player_name, "En-J1n"],
