...
class AidaGame:
    def __init__(self, bot, interaction, thread, player_name, round_limit=20, winning_score=300):
        """
        Initializes the AidaGame instance.
        """
        self.bot = bot
    ...
    def roll_die(self):
        ...
        """
        roll = random.choices([1, 2, 3, 4, 5, 6], weights=[10, 18, 18, 18, 18, 18])[0]
        return roll

    def aida_roll_die(self):
        """
    ...
    def handle_bearish(self, card, is_player=True):
        ...
        """
        roll = self.roll_die() if is_player else self.aida_roll_die()
        dodged = roll % 2 == 0
        ...
        if dodged:
            ...
            if card.penalty == "Half":
                ...
                if is_player:
                    ...
                    self.aida_score = max(0, self.aida_score - 10)
    ...
    def aida_play_turn(self):
        """
        Handles Aida's turn logic.
        """
        self.aida_turn_score = 0
        actions = []
        ...
        while True:
            ...
            if card.type == "Bearish":
                ...
                if not success:
                    actions.append(f"Aida drew a {card.name} (Bearish) and rolled a {roll} → {phrase}. Turn ends.")
                    self.aida_turn_score = 0
                    break
                else:
                    actions.append(f"Aida drew a {card.name} (Bearish) and rolled a {roll} → {phrase}.")
                    continue

            roll = self.aida_roll_die()

            if roll == 1:
                actions.append(f"Aida drew a {card.name} ({card.value} pts) and rolled a 1 → Busted! Turn ends.")
                self.aida_turn_score = 0
                break
            else:
                self.aida_turn_score += card.value
                actions.append(f"Aida drew a {card.name} ({card.value} pts) and rolled a {roll} → Turn score: {self.aida_turn_score}")

                # Aida's calculated stacking logic
                behind_by = self.player_score - self.aida_score

                if behind_by > 30:
                    # Aida takes calculated risks when far behind
                    risk_roll = random.uniform(0, 100)
                    if risk_roll > 40:  # 60% chance to continue rolling
                        continue
                    else:
                        actions.append("Aida decides to stack to catch up.")
                        self.aida_score += self.aida_turn_score
                        break
                elif self.aida_turn_score >= 40:
                    # Aida stacks when her turn score reaches 40 or more
                    actions.append("Aida stacks her sats at 40 or more.")
                    self.aida_score += self.aida_turn_score
                    break
                elif 21 <= self.aida_turn_score < 40:
                    # Aida uses a calculated risk factor for scores between 21 and 39
                    risk_roll = random.uniform(0, 100)
                    if risk_roll > 50:  # 50% chance to continue rolling
                        continue
                    else:
                        actions.append("Aida decides to stack early.")
                        self.aida_score += self.aida_turn_score
                        break
                else:
                    # Continue rolling for lower scores
                    continue

        return actions, self.aida_score >= self.winning_score
    ...
    def is_game_over(self):
        ...
        return (
            ...
            self.aida_score >= self.winning_score or
    ...
    def get_winner(self):
        ...
        """
        if self.player_score > self.aida_score:
            return "player", "You win!"
        elif self.aida_score > self.player_score:
            return "aida", "Aida wins!"
        else:
    ...
    async def conclude_game(self):
        ...
        result_message = (
            ...
            f"🤖 Aida wins! {phrase}" if winner == "aida" else
            "🤝 It's a tie!"
        )
        ...
        try:
            await post_results_to_channel(
                bot=self.bot,
                winner_name=self.player_name if winner == "player" else "Aida",
                total_score=max(self.player_score, self.aida_score),
                game_mode="Aida",
                player_names=[self.player_name, "Aida"],
