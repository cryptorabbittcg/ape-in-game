...
class SandyGame:
    ...
    def sandy_play_turn(self):
        ...
        while True:
            ...
            if card.type == "Bearish":
                ...
                if not success:
                    actions.append(f"Sandy drew a {card.name} (Bearish) and rolled a {roll} → {phrase}. Turn ends.")
                    self.sandy_turn_score = 0
                    break
                else:
                    actions.append(f"Sandy drew a {card.name} (Bearish) and rolled a {roll} → {phrase}.")
                    continue

            roll = self.sandy_roll_die()

            if roll == 1:
                actions.append(f"Sandy drew a {card.name} ({card.value} pts) and rolled a 1 → Rekt! Turn ends.")
                self.sandy_turn_score = 0
                break
            else:
                self.sandy_turn_score += card.value
                actions.append(f"Sandy drew a {card.name} ({card.value} pts) and rolled a {roll} → Turn score: {self.sandy_turn_score}")
    ...
    async def conclude_game(self):
        ...
        result_message = (
            ...
            f"🤖 Sandy wins! {phrase}" if winner == "sandy" else
            "🤝 It's a tie!"
        )
