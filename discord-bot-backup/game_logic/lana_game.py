...
class LanaGame:
    def __init__(self, bot, interaction, thread, player_name, round_limit=15, winning_score=200):
        """
        Initializes the LanaGame instance.
        """
        self.bot = bot
    ...
    def lana_roll_die(self):
        """
        Rolls a weighted die for Lana.
        """
    ...
    def handle_bearish(self, card, is_player=True):
        ...
        """
        roll = self.roll_die() if is_player else self.lana_roll_die()
        dodged = roll % 2 == 0
        ...
        if dodged:
            ...
            if card.penalty == "Reset":
                ...
                if is_player:
                    ...
                    self.lana_score = max(0, self.lana_score - 10)
    ...
    def lana_play_turn(self):
        """
        Handles Lana's turn logic.
        """
        self.lana_turn_score = 0
        actions = []
        ...
        while True:
            ...
            if card.type == "Bearish":
                ...
                if not success:
                    actions.append(f"Lana drew a {card.name} (Bearish) and rolled a {roll} → {phrase}. Turn ends.")
                    self.lana_turn_score = 0
                    break
                else:
                    actions.append(f"Lana drew a {card.name} (Bearish) and rolled a {roll} → {phrase}.")
                    continue

            roll = self.lana_roll_die()

            if roll == 1:
                actions.append(f"Lana drew a {card.name} ({card.value} pts) and rolled a 1 → Busted! Turn ends.")
                self.lana_turn_score = 0
                break
            else:
                self.lana_turn_score += card.value
                actions.append(f"Lana drew a {card.name} ({card.value} pts) and rolled a {roll} → Turn score: {self.lana_turn_score}")

                if self.lana_turn_score >= 30:
                    risk_roll = random.uniform(0, 100)
                    if risk_roll > 30:
                        actions.append("Lana stacks her sats.")
                        self.lana_score += self.lana_turn_score
                        break
                    else:
                        continue
                else:
        ...
        return actions, self.lana_score >= self.winning_score
    ...
    def is_game_over(self):
        ...
        return (
            ...
            self.lana_score >= self.winning_score or
    ...
    def get_winner(self):
        ...
        """
        if self.player_score > self.lana_score:
            return "player", "You win!"
        elif self.lana_score > self.player_score:
            return "lana", "Lana wins!"
        else:
    ...
    async def conclude_game(self):
        ...
        result_message = (
            ...
            f"🤖 Lana wins! {phrase}" if winner == "Lana" else
            "🤝 It's a tie!"
        )
        ...
        try:
            await post_results_to_channel(
                bot=self.bot,
                winner_name=self.player_name if winner == "player" else "Lana",
                total_score=max(self.player_score, self.lana_score),
                game_mode="Lana",
                player_names=[self.player_name, "Lana"],
